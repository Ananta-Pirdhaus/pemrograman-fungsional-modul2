...
# TODO 5 Tambahkan fungsi find_expense disini
find_expense = lambda expenses, description: [expense for expense in expenses if description.lower() in expense['deskripsi'].lower()]

# TODO 7 Tambahkan fungsi edit_expense disini
edit_expense = lambda expenses, index, date, description, amount: expenses[:index] + [{'tanggal': date, 'deskripsi': description, 'jumlah': amount}] + expenses[index+1:]
...
def find_and_display_expense(expenses):
    description = input("Masukkan deskripsi yang ingin dicari: ")
    found_expenses = find_expense(expenses, description)
    if found_expenses:
        print("\nPengeluaran yang ditemukan:")
        for expense in found_expenses:
            print(f"{expense['tanggal']} - {expense['deskripsi']} - Rp {expense['jumlah']}")
    else:
        print("Tidak ada pengeluaran dengan deskripsi tersebut.")

def edit_expense_interactively(expenses):
    index = int(input("Masukkan indeks pengeluaran yang ingin diedit: "))
    date = input("Masukkan tanggal baru (YYYY-MM-DD): ")
    description = input("Masukkan deskripsi baru: ")
    amount = int(input("Masukkan jumlah baru: "))
    expenses = edit_expense(expenses, index, date, description, amount)
...
def main():
    ...
    while True:
        display_menu()
        choice = get_user_input("Pilih menu (1/2/3/4/5/6/7): ")
        if choice == 1:
            ...
            expenses = edit_expense_interactively(expenses)
        elif choice == 7:
